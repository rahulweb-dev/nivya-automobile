...
export default function PostVehicle() {
  ...
  const brandOptions = ["Hyundai", "Maruti Suzuki", "Tata", "Mahindra", "Kia", "Toyota"];
  ...
  return (
    <div className="min-h-screen p-6 mt-24 bg-gradient-to-br from-gray-50 to-gray-100">
      <Head>
      ...
      <main className="max-w-6xl mx-auto">
        <header className="mb-6">
          <h1 className="text-4xl font-extrabold text-gray-900">Post Your Vehicle for Sale</h1>
          <p className="mt-1 text-sm text-gray-500">
            Fill out the required fields (*) below to list your vehicle. You can save draft at any
            time.
          </p>
        </header>
        ...
        <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <section className="p-6 bg-white shadow-sm rounded-2xl">
              <h2 className="mb-4 text-lg font-semibold text-gray-700">Vehicle Information</h2>

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <label className="flex flex-col">
                  <span className="text-sm font-medium text-gray-600">Basic Type *</span>
                  <select
                    name="bodyType"
                    value={form.bodyType}
                ...
                <label className="flex flex-col">
                  <span className="text-sm font-medium text-gray-600">Vehicle Category *</span>
                  <select
                    name="brand"
                    value={form.brand}
                    onChange={handleChange}
                ...
                <label className="flex flex-col">
                  <span className="text-sm font-medium text-gray-600">Vehicle Status *</span>
                  <input
                    name="userType"
                    value={form.userType}
                    onChange={handleChange}
                ...
                <label className="flex flex-col">
                  <span className="text-sm font-medium text-gray-600">Model Year</span>
                  <input
                    name="registeredCity"
            ...
            {/* Right card */}
            <section className="p-6 bg-white shadow-sm rounded-2xl">
              <h2 className="mb-4 text-lg font-semibold text-gray-700">Condition & Status</h2>

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <label className="flex flex-col">
                  <span className="text-sm font-medium text-gray-600">Mileage / Kms Driven *</span>
                  <input
                    name="mileage"
                    onChange={handleChange}
                ...
                <label className="flex flex-col">
                  <span className="text-sm font-medium text-gray-600">Engine Capacity</span>
                  <input
                    name="engine"
                    onChange={handleChange}
                    placeholder="CC"
                ...
                <label className="flex flex-col sm:col-span-2">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    {/* small color circles */}
                    {["#e11d48", "#111827", "#374151", "#9CA3AF", "#ffffff"].map((c, idx) => (
                      <button
                        key={idx}
                        type="button"
                ...
                <label className="flex flex-col sm:col-span-2">
                  <span className="text-sm font-medium text-gray-600">Location & Pricing</span>
                  <input
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div className="p-6 bg-white shadow-sm md:col-span-2 rounded-2xl">
              <h3 className="mb-3 font-semibold text-gray-700">Upload Images</h3>
              ...
              >
                ...
                <p className="mt-1 text-xs text-gray-400">Recommended min resolution: 1200x800px. Max per file: 5MB</p>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept="image/*"
                      ...
                      <div className="flex items-center justify-between px-2 py-1 text-xs bg-white/80">
                        <button
                          ...
                          className={`px-2 py-1 text-xs rounded ${
                            img.isPrimary ? "bg-blue-600 text-white" : "bg-white text-gray-700"
                          }`}
                        >
                          {img.isPrimary ? "Primary Image" : "Set Primary"}
                        </button>
                        <span className="text-gray-600 truncate max-w-[70px]">{img.name}</span>
                      </div>
                    </div>
                  ))
                )}
            ...
            <aside className="p-6 bg-white shadow-sm rounded-2xl">
              <h3 className="mb-4 text-lg font-semibold text-gray-700">Accessories & Features</h3>
              <p className="mb-3 text-sm text-gray-400">Select the features available in your vehicle.</p>
          ...
          <div className="flex items-center justify-between px-2 py-4">
            ...
            <div className="text-sm text-gray-500">
              <span className="font-medium text-gray-700">Images:</span> {form.images.length}/{MAX_IMAGES}
            </div>
          </div>

          {message && (
            <div className="p-3 text-sm text-white bg-green-600 rounded">{message}</div>
          )}
        </form>
      </main>
    </div>
