...
export default function TrueValueSell() {
  ...
  const validate = () => {
    ...
    if (!/^[A-Z]{2}\d{2}[A-Z]{1,2}\d{4}$/i.test(form.regNumber))
      newErrors.regNumber = "Enter valid registration number (e.g. MH12AB1234).";
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="absolute inset-0 flex items-center justify-center px-4 sm:px-8">
          <div className="flex flex-col items-center justify-between w-full max-w-5xl gap-6 p-6 shadow-2xl bg-white/60 backdrop-blur-md sm:p-10 rounded-2xl sm:flex-row">
            <h1 className="text-2xl font-extrabold leading-snug text-center text-blue-900 uppercase sm:text-4xl sm:text-left">
              Gaadi Bikti Hai <span className="text-red-600">Sirf TrueValue Pe</span>
            </h1>
      ...
      <section className="max-w-5xl p-6 mx-auto my-16 bg-white border border-gray-100 shadow-2xl sm:p-10 rounded-3xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8 sm:grid-cols-3">
          {/* Name */}
          ...
          {/* Email */}
          <div className="flex flex-col">
            <label className="mb-2 font-medium text-gray-600">Email Address</label>
            <input
              type="email"
              name="email"
              value={form.email}
          ...
          {/* number */}
          <div className="flex flex-col">
            <label className="mb-2 font-medium text-gray-600">Mobile Number*</label>
            <input
              type="tel"
              name="number"
              value={form.number}
          ...
          <div className="flex flex-col sm:col-span-3">
            <label className="mb-2 font-medium text-gray-600">Registration Number*</label>
            <input
              type="text"
              name="regNumber"
          ...
          <div className="flex items-start gap-3 sm:col-span-3">
            <input
              ...
              onChange={handleChange}
              id="authorize"
              className={`w-5 h-5 mt-1 ${errors.authorize ? "border-red-500" : ""}`}
            />
            <label htmlFor="authorize" className="text-sm text-gray-600">
              I authorize Maruti Suzuki India Ltd. or its partners to call me. Please read our{" "}
          ...
          {errors.authorize && (
            <p className="text-sm text-red-500 sm:col-span-3">{errors.authorize}</p>
          )}
